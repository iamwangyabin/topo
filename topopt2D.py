...
# MAIN DRIVER
def main(volfrac, penal, rmin, ft, chg, fldr,savfolder):
    ...
    while change > tol and loop < maxloops:
        ...
        fig.canvas.draw()
        mkdir(str(folder)+'/anim/')
        fig.savefig(str(folder)+'/anim/{0}.png'.format(loop))
    fig.savefig(str(savfolder) + str(volfrac)+'_'+str(penal)+'_'+str(rmin)+'.png')
    plt.clf()
    plt.cla()
    plt.close('all')
...
if __name__ == "__main__":
    ...
    volfrac = 0.3
    rmin = 3.5  # lower number: more branching (initial: 5.4, try 2.0 or 1.5) proposal: 0.04 * nelx
    penal = 5.0  # ensure black and white solution
